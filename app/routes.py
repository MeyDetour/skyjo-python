...
def update_board():
    done, players_cards, discard_cards_left ,turn ,turn_number= game.run()
    ...
    if showed_card :
        showed_card = game.get_html_for_card( showed_card[0],showed_card[1],turn_number>0 and not staging_card,turn,-2,'discardedCard')
    if staging_card :
    ...
    if done :
        ...
        for idx, player_card_columns in enumerate(players_cards):
            ...
            if idx == turn:
                player_cards += "<span class='turn'>Your turn</span>"
            ...
            for column in player_card_columns:
                ...
                for card in column:
                    cards_column += game.get_html_for_card(card[0], card[1], (idx == turn and not staging_card and turn_number == 0)  or staging_card or has_the_player_passed_staging_card ,idx,(column_number*card_number),'')
                    card_number += 1
                # close the column
                cards_column += "</div>"
